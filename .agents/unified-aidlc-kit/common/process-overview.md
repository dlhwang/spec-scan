...
• **CONSTRUCTION PHASE**: Design, implementation, build and test (per-unit design + Micro-Loop + Code Generation + Build & Test)
• **OPERATIONS PHASE**: Placeholder for future deployment and monitoring workflows

## The Adaptive Workflow:
• **Workspace Detection** (always) → **Reverse Engineering** (brownfield only) → **Requirements Analysis** (always, adaptive depth) → **Conditional Phases** (as needed) → **Workflow Planning** (always) → **Per-Unit Micro-Loop** (Deep Interview → Consensus Planning → Goal-Driven Execution) → **Code Generation** (always, per-unit) → **Build and Test** (always)
...
• **Micro-Loop Stage**: A 3-step loop executed for each Unit during CONSTRUCTION (Deep Interview Gating, Consensus Planning, Goal-Driven Execution)
...
flowchart TD
    ...
    subgraph CONSTRUCTION["🟢 CONSTRUCTION PHASE"]
        ...
        subgraph MICROLOOP["🔄 Micro-Loop (Per-Unit)"]
            DI["Deep Interview Gating<br/><b>ALWAYS</b>"]
            CP["Consensus Planning<br/><b>CONDITIONAL</b>"]
            GDE["Goal-Driven Execution<br/><b>ALWAYS</b>"]
        end
    ...
    style DI fill:#42A5F5,stroke:#1565C0,stroke-width:3px,color:#fff
    style CP fill:#AB47BC,stroke:#6A1B9A,stroke-width:3px,stroke-dasharray: 5 5,color:#fff
    style GDE fill:#42A5F5,stroke:#1565C0,stroke-width:3px,color:#fff
    style MICROLOOP fill:#E3F2FD,stroke:#1565C0,stroke-width:2px,color:#000
...
- **🔄 Micro-Loop (Per-Unit, 3 Stages)**:
  - **Deep Interview Gating**: Detects ambiguity/conflicts between codebase and requirements, and solidifies specs via developer interviews (**ALWAYS** - executes for all Units).
  - **Consensus Planning**: Drafts a consensus-based implementation plan and verification criteria based on interview specs (**CONDITIONAL** - based on complexity).
  - **Goal-Driven Execution**: Decomposes the plan into a Goal Tree and executes/verifies using an evidence-backed state machine (**ALWAYS** - executes for all Units).
...
- **Evidence-Based Completion**: Goal completion (`complete`) is allowed only if objective verification evidence (test logs, assertion outcomes) is written in the `evidence` field. Bypassing evidence is prohibited.
