...
## Trade-off Aware Question Format

**Purpose**: When presenting design choices to developers, specify the pros, cons, and downstream impacts of each option to enable informed decision-making.

### Application Triggers

This format MUST be used in the following scenarios:
- Conflicts between existing rules (validation logic, data schemas) and new requirements.
- Choices affecting compatibility, performance, or long-term maintenance.
- Ambiguity detected during the Deep Interview Gating stage.
- Architectural choices affecting subsequent development steps.

### Trade-off Question Structure

Include **Pros**, **Cons**, and **Impact Analysis** (Compatibility, Performance, Maintainability) for each option:

```markdown
## Trade-off Question [Number]
[Detailed description of the conflict or ambiguity]
...
Include at least the following impact metrics for each option:

| Metric | Description | Evaluation Example |
|:---|:---|:---|
| **Compatibility** | Impact on existing code, APIs, or database schemas. | Backward compatible / Breaking changes |
| **Performance** | Runtime speed, compilation overhead, or memory footprint. | No impact / Minor overhead / High impact |
| **Maintainability** | Code complexity, technical debt, ease of future changes. | Improved / Neutral / Added debt |
| **Implementation Cost** | Expected engineering effort, difficulty, and dependencies. | Low / Medium / High |
| **Risk** | Outage probability, rollback difficulty, regression risk. | Low / Medium / High |

### Example

```markdown
## Trade-off Question 1
When restoring @ModelAttribute OrderRequest, how should we maintain compatibility with existing validationConditions data format?

### Option A: Keep existing schema and append new fields (Recommended)
Preserves existing validationConditions JSON Paths and structures, only adding new parameters.
- **Pros**: Existing verification logic runs without modifications; enables gradual migration.
- **Cons**: Schema size increases.
- **Impact Analysis**:
  - Compatibility: ✅ Fully backward compatible
  - Performance: ⚠️ Minor parsing overhead from additional fields
  - Maintainability: ⚠️ Minor tech debt from legacy fields
  - Implementation Cost: Low
  - Risk: Low

### Option B: Complete refactoring based on OrderRequest spec
Redesigns the validationConditions schema to match the OrderRequest parameters.
- **Pros**: Clean data structure; improves long-term maintainability.
- **Cons**: Requires rewriting all verification consumers; requires migration scripts.
- **Impact Analysis**:
  - Compatibility: ❌ Breaking change (affects existing clients)
  - Performance: ✅ Optimized schema improves runtime latency
  - Maintainability: ✅ Clears tech debt; improves readability
  - Implementation Cost: High (Refactoring + Migration)
  - Risk: High (Potential regression bugs)

[Answer]: Option A
[Rationale]: 
```

### Trade-off Question Workflow

1. **Detect**: Stop coding immediately upon detecting design conflicts or ambiguities.
2. **Design**: Establish 2 to 4 realistic design options.
3. **Analyze**: Evaluate impact metrics (Compatibility, Performance, Maintainability, Cost, Risk) for each option.
4. **Draft**: Create `{phase-name}-tradeoff-questions.md` using the trade-off format.
5. **Answer**: Wait for developer selection and rationale.
6. **Save**: Record the approved decision in `specs/deep-interview-[slug].md` as Decisions/Constraints.

### Usage Classification Matrix

| Context | Question Format to Use |
|:---|:---|
| Simple factual checks (deployment target, basic parameters) | Standard Multiple Choice Format |
| Design decisions with varying technical trade-offs | **Trade-off Aware Question Format** |
| Conflict detected with existing codebase rules | **Trade-off Aware Question Format** |
| Ambiguity triggered during Deep Interview Gating | **Trade-off Aware Question Format** |
...
- ✅ **Use trade-off aware question format for design conflicts/ambiguity (MANDATORY)**
- ✅ **Include impact analysis (Compatibility, Performance, Maintainability, Cost, Risk) in trade-offs**
...
- ❌ **Do not present design choices without impact analysis**
